...
class WorkerStatsScreen extends StatelessWidget {
        ...
        child: SafeArea(
          child: StreamBuilder<List<TicketModel>>(
            ...
            builder: (context, snapshot) {
              ...
              final assignedTickets = snapshot.data ?? [];
              final ticketService = TicketService();
              final userService = UserService();
              final allTicketsStream = ticketService.getAllTickets();
              
              return StreamBuilder<List<TicketModel>>(
                stream: allTicketsStream,
                builder: (context, allSnapshot) {
                  return StreamBuilder(
                    stream: userService.getAllUsers(),
                    builder: (context, userSnapshot) {
                      final allTickets = allSnapshot.data ?? [];
                      
                      // Estadísticas de tickets
                      final openTickets = allTickets.where((t) => t.status == 'open').length;
                      final inProgressTickets = assignedTickets.where((t) => t.status == 'in_progress').length;
                      final closedTickets = assignedTickets.where((t) => t.status == 'closed' || t.status == 'resolved').length;
                      final myOpenTickets = assignedTickets.where((t) => t.status == 'open').length;
                      final totalAssigned = assignedTickets.length;
                  ...
                  return SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Gráfica de distribución de tickets asignados
                        _buildStatusChart(myOpenTickets, inProgressTickets, closedTickets, totalAssigned, isDark),
                        const SizedBox(height: 24),
                        
                        // Gráfica comparativa sistema vs mis tickets
                        _buildComparisonChart(openTickets, myOpenTickets, isDark),
                        ...
                        if (assignedTickets.isNotEmpty) ...[
                          ...
                          const SizedBox(height: 16),
                          ...assignedTickets.take(5).map((ticket) => Padding(
                ...
                Expanded(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 8),
                      _buildLegendItem('Resueltos', AppTheme.success, closed, isDark),
                    ],
                  ),
                ),
              ],
  ...
  Widget _buildComparisonChart(int systemOpen, int myOpen, bool isDark) {
    final maxValue = [systemOpen, myOpen]
            .reduce((a, b) => a > b ? a : b)
            .toDouble() *
        1.2;
        ...
        children: [
          Row(
            children: [
              ...
              Flexible(
                child: Text(
                  'Comparación: Sistema vs Mis Tickets',
                  style: GoogleFonts.inter(
                    fontSize: 20,
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
